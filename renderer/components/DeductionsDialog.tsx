...
export const DeductionsDialog: React.FC<DeductionsDialogProps> = React.memo(
  ({
    ...
    useEffect(() => {
      ...
      if (isOpen && !hasLoadedAdvances) {
        loadUnpaidAdvances();
      }
    }, [isOpen, loadUnpaidAdvances, hasLoadedAdvances]);

    // Reset hasLoadedAdvances when the dialog closes
    useEffect(() => {
      if (!isOpen) {
    ...
    }, [selectedAdvances, unpaidAdvances, deductionAmounts, isCalculating]);
