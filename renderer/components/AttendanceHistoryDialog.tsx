...
interface AttendanceHistoryDialogProps {
  ...
  dbPath?: string; // Make optional, only used for desktop
  companyName?: string; // Add companyName for Firestore
  isWebMode: boolean; // Add flag to determine environment
  onRevertAttendance: (
    day: number,
    timeIn: string | null,
    timeOut: string | null
  ...
  onRevertCompensation,
    ...
    useEffect(() => {
      const fetchHistory = async () => {
        // Check for required props based on mode
        if (!isOpen || !employeeId || !year || !month || !day) {
          setCombinedEntries([]);
          return;
        }
        if (!isWebMode && !dbPath) {
          setError("Database path is required for desktop mode history.");
          return;
        }
        if (isWebMode && !companyName) {
          setError("Company name is required for web mode history.");
          return;
        }
        ...
        if (isWebMode) {
          // TODO: Implement Firestore history fetching logic
          console.warn(
            "[AttendanceHistoryDialog] Firestore history fetching not yet implemented."
          );
          // Example structure (needs actual implementation):
          // const attendanceHistory = await loadAttendanceHistoryFirestore(companyName, employeeId, year, month, day);
          // const compensationHistory = await loadCompensationHistoryFirestore(companyName, employeeId, year, month, day);
          // ... process and combine history ...
          setError("History view is not yet available in web mode.");
          setCombinedEntries([]);
        } else {
          // Desktop mode: Fetch from local files (existing logic)
      ...
      fetchHistory();
    }, [isOpen, employeeId, year, month, day, dbPath, companyName, isWebMode]);
