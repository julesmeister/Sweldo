...
import { IoLockClosed, IoEyeOutline, IoEyeOffOutline, IoChevronDown, IoSyncOutline } from "react-icons/io5";
...
export const LoginDialog: React.FC<LoginDialogProps> = ({ onSuccess }) => {
  ...
  const handleRecoverSettings = async () => {
    try {
      ...
      if (success) {
        toast.success("Settings recovered successfully!");
        // Update local state if company name was recovered
        const recoveredCompanyName = useSettingsStore.getState().companyName;
        if (recoveredCompanyName && recoveredCompanyName !== selectedCompany) {
          setSelectedCompany(recoveredCompanyName);
          setCompanyName(recoveredCompanyName);
        }
      } else {
        toast.error("No settings backup found to recover from.");
      }
    } catch (error) {
      console.error("[LoginDialog] Settings recovery failed:", error);
      toast.error("Failed to recover settings");
    }
  };
        ...
        <div className="flex flex-col items-center justify-center p-12">
          ...
          {companySelectionOnly ? (
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-4">
              ...
              {/* Add a recovery button in web mode right above the login button */}
              {!isWebEnvironment() && (
                <div className="mt-2 mb-4">
                  <button
                    type="button"
                    onClick={handleRecoverSettings}
                    className="w-full flex items-center justify-center py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                  >
                    <IoSyncOutline className="w-4 h-4 mr-2" />
                    Recover Settings
                  </button>
                  <p className="text-xs text-gray-500 mt-1 text-center">
                    If company name or other settings were lost
                  </p>
                </div>
              )}
