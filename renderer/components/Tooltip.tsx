...
export const Tooltip: React.FC<TooltipProps> = ({
  ...
  const tooltipClassName = `bg-gray-800 text-white text-sm px-4 py-2 rounded-lg shadow-lg min-w-[${width}] whitespace-normal`;
      ...
      {isVisible && (
        <div className={`absolute ${positions[position]} z-50`}>
          <div className="relative">
            ...
            <div
              className={`absolute w-3 h-3 transform rotate-45 bg-gray-800 ${arrowPositions[position]}`}
            />
          </div>
        </div>
      )}
