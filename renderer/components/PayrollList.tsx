...
export const PayrollList: React.FC<PayrollListProps> = ({ payrolls: initialPayrolls, onSelectPayroll, onPayrollDeleted, selectedEmployeeId, dbPath, employee }) => {
  const [filterType, setFilterType] = useState<'3months' | '6months' | 'year' | 'custom'>('3months');
  ...
  useEffect(() => {
    const loadPayrolls = async () => {
      ...
      if (filterType === 'custom' && month) {
        ...
        const payrollData = await Payroll.loadPayrollSummaries(dbPath, selectedEmployeeId, year, month.getMonth() + 1);
        ...
        const monthsMap: Record<Exclude<typeof filterType, 'custom'>, number> = {
          '3months': 3,
          '6months': 6,
          'year': 12
        };
        ...
        const payrollData = await Payroll.loadPayrollSummaries(dbPath, selectedEmployeeId, year, month ? month.getMonth() + 1 : 1);
        filteredPayrolls = payrollData.map(payroll => ({
          ...payroll,
          employeeName: employee?.name || 'Unknown Employee',
        })).filter(payroll => {
    ...
    <div className="bg-white rounded-lg shadow overflow-visible z-10">
      {selectedPayroll && (
        <div className="relative z-10">
          <PayrollSummary 
            data={{...selectedPayroll, employeeName: employee?.name || 'Unknown Employee'}} 
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2">
          <div className="flex items-center bg-white rounded-lg border border-gray-200 p-1">
            <button
              ...
              className={`px-3 py-1 text-sm rounded-md transition-colors duration-150 ${
                filterType === '3months' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'
              }`}
            >
              Last 3 Months
            </button>
            <button
              ...
              className={`px-3 py-1 text-sm rounded-md transition-colors duration-150 ${
                filterType === '6months' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'
              }`}
            >
              Last 6 Months
            </button>
            <button
              ...
              className={`px-3 py-1 text-sm rounded-md transition-colors duration-150 ${
                filterType === 'year' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'
              }`}
            >
              Last Year
            </button>
          ...
          <div className="relative overflow-visible">
            ...
            >
              {filterType === 'custom' 
                ? month?.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) || 'Select Month'
                : 'Select Month'}
            </button>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button
                    onClick={async (e) => {
                      e.stopPropagation();
                      try {
                        await Payroll.deletePayrollSummary(dbPath, employee!.id, payroll.startDate, payroll.endDate).then(() => {
                          onPayrollDeleted(); // Call the function to refresh payrolls
                        });
                        toast.success('Payroll summary deleted successfully.');
                      } catch (error) {
                        toast.error('Error deleting payroll summary.');
                      }
                    }}
      ...
      {payrolls.length === 0 && (
        <div className="text-center py-12 px-4">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">No payrolls generated yet</h3>
          <p className="mt-1 text-sm text-gray-500">
            Select a date range and click 'Generate Payroll' to create a new payroll record.
          </p>
        </div>
      )}
    </div>
