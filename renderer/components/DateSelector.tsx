...
export default function DateSelector() {
  ...
  return (
    <div className="relative inline-block text-left" 
      ...
      <div>
      <button className={` rounded-md px-3 py-1 text-sm font-medium  ${isOpen ? 'bg-white text-black' : 'text-white hover:bg-white hover:text-black'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-500 transition-all duration-200 inline-flex items-end flex-col text-left leading-none`}>
          <span className="text-sm" >{years[selectedYear - 2024] || '2025'}</span> {/* Display the selected year or a default text */}
          <span className="text-lg font-bold pb-1" style={{ lineHeight: '0.8' }}>{months[selectedMonth]}</span> {/* Display the selected month in larger font */}
        </button>
      </div>
      {isOpen && (
        <div className="relative">
          <div className="absolute right-0 z-10 mt-2 w-60 rounded-md shadow-lg bg-white">  
            ...
            <div className="grid grid-cols-2 gap-2 p-2 max-h-[200px]" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
