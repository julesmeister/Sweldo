...
export async function migrateCsvToJson(
  ...
  try {
    ...
    // After successful migration, delete the CSV file
    onProgress?.("Starting cleanup of employee CSV file...");
    await deleteCsvFiles(path.dirname(csvFilePath), "employees", onProgress);
