...
export default function LeavesPage() {
  ...
  useEffect(() => {
    const loadEmployeeAndLeaves = async () => {
      ...
      try {
        // Load employee
        console.log('[LeavesPage] Loading employee with ID', selectedEmployeeId);
  ...
  const [clickPosition, setClickPosition] = useState<{ top: number; left: number; showAbove: boolean; caretLeft: number } | null>(null);

  const handleNewLeaveClick = (event: React.MouseEvent) => {
  ...
  const handleRowClick = (leave: Leave, event: React.MouseEvent) => {
    event.stopPropagation();
    const rect = event.currentTarget.getBoundingClientRect();
    const windowHeight = window.innerHeight;
    const windowWidth = window.innerWidth;
    const dialogHeight = 450; // Adjusted height of dialog
    const dialogWidth = 450; // Adjusted width to match the form's actual width
    const spacing = 8; // Space between dialog and trigger
    ...
    // Position the dialog
    const top = showAbove 
      ? rect.top - spacing  // When showing above, align with the top of the row
      : rect.bottom + spacing; // When showing below, align with the bottom of the row
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h2 className="text-lg font-medium text-gray-900">
                  {selectedEmployeeId ? `${employee?.name}'s Leave Requests` : 'Leave Requests'}
                </h2>
                <button
                  type="button"
                  ...
                  {leaves.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 px-4">
                      <div className="text-center">
                        ...
                        <h3 className="mt-2 text-sm font-semibold text-gray-900">No leave requests found</h3>
                        ...
                        {leaves.map((leave) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${getTypeColor(leave.type)}`}>
                                {leave.type}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              <span className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${getStatusColor(leave.status)}`}>
                                {leave.status}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
