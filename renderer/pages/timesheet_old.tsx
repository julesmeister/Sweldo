  ...
  const refreshTimesheetData = async (showToast: boolean = true) => {
    ...
    try {
      ...
      if (isWebEnvironment()) {
        ...
        console.log("Using companyName:", companyName);
        ...
        if (isWebEnvironment()) {
          ...
          console.log("DEBUG - Timesheet Refresh - Web Mode:");
          console.log("- Company Name:", companyName);
          console.log("- Employee ID:", selectedEmployeeId);
          console.log("- Year:", year);
          console.log("- Month:", storedMonthInt);

          try {
            console.log("- Attempting to load attendance (refresh)...");
            attendanceData = await loadAttendanceFirestore(selectedEmployeeId, year, storedMonthInt, companyName);
            console.log("- Attendance data loaded:", attendanceData.length, "records");

            console.log("- Attempting to load compensation (refresh)...");
            compensationData = await loadCompensationFirestore(selectedEmployeeId, year, storedMonthInt, companyName);
            console.log("- Compensation data loaded:", compensationData.length, "records");
          } catch (error) {
            console.error("- Firestore refresh error:", error);
            throw error;
          }
        }
      } else {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (isWebEnvironment()) {
          ...
          console.log("DEBUG - Timesheet Web Mode Loading Data:");
          console.log("- Company Name:", companyName);
          console.log("- Employee ID:", selectedEmployeeId);
          console.log("- Year:", year);
          console.log("- Month:", storedMonthInt);
          ...
          try {
            console.log("- Attempting to load attendance...");
            attendanceData = await loadAttendanceFirestore(selectedEmployeeId, year, storedMonthInt, companyName);
            console.log("- Attendance data loaded:", attendanceData.length, "records");
            console.log("- Sample attendance:", attendanceData.length > 0 ? JSON.stringify(attendanceData[0]) : "no data");

            console.log("- Attempting to load compensation...");
            compensationData = await loadCompensationFirestore(selectedEmployeeId, year, storedMonthInt, companyName);
            console.log("- Compensation data loaded:", compensationData.length, "records");
            console.log("- Sample compensation:", compensationData.length > 0 ? JSON.stringify(compensationData[0]) : "no data");
          } catch (error) {
