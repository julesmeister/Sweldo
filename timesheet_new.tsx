...
const TimesheetPage: React.FC = () => {
    ...
    try {
      ...
      if (isWebEnvironment()) {
        ...
        if (!companyName) {
          ...
          console.error("[refreshTimesheetData] ERROR: Company name is not set for web mode:", companyName);
          return;
        }

        console.log("[refreshTimesheetData] PARAMS - Using company name:", companyName);
        console.log("[refreshTimesheetData] PARAMS - Employee ID:", selectedEmployeeId);
        console.log("[refreshTimesheetData] PARAMS - Year:", year);
        console.log("[refreshTimesheetData] PARAMS - Month:", storedMonthInt);

        try {
          console.log("[refreshTimesheetData] Fetching attendance and compensation data from Firestore...");
          ...
          console.log("[refreshTimesheetData] RESULTS - Attendance data count:", attendanceData.length);
          console.log("[refreshTimesheetData] RESULTS - Compensation data count:", compensationData.length);

          if (attendanceData.length === 0) {
            console.warn("[refreshTimesheetData] WARNING: No attendance data found for this employee/period");
          }

          if (compensationData.length === 0) {
            console.warn("[refreshTimesheetData] WARNING: No compensation data found for this employee/period");
          }
        } catch (error) {
          console.error("[refreshTimesheetData] ERROR during Firestore data fetch:", error);
        }
      } else {
  ...
  // Restore the memoization hooks that were incorrectly removed
  ...
  // Fix the scheduleMap call that had incorrect parameters
        ...
        {columns.map(
          (column) =>
            ...
            (column.key === "timeIn" || column.key === "timeOut" ? (
              ...
              <td
                key={column.key}
                className={`${column.key === "day" ? "sticky left-0 z-10" : ""
                    ...
                    {hasAccess("MANAGE_PAYROLL") && (
                      ...
                      >
                        ...
                        <svg
                          ...
                          className={`h-5 w-5 transition-transform duration-300 ${isRecomputing ? "rotate-180" : ""
                            }`}
                          viewBox="0 0 20 20"
                          fill="currentColor"
                        >
