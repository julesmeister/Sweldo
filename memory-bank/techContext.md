...
- **`renderer/scripts/generate-tailwind.js`**: Node.js script using `postcss`, `tailwindcss`, and `autoprefixer` to process `renderer/styles/globals.css` (with `renderer/tailwind.config.js`) and generate the complete `tailwind-web.css` file for web deployments. Invoked via `npm run generate:tailwind`.
...
1. Build:
   ...
   yarn build
   ```
   Compiles main/renderer code for desktop.
   ...
   yarn build:web
   ```
   Builds the web version. This script (defined in `package.json`) orchestrates several steps including:
   - `npm run setup:web` (copies initial resources)
   - `npm run generate:tailwind` (generates `renderer/public/styles/tailwind-web.css`)
   - `npm run sync:styles` (prepares supplementary styles for `styleInjector.js`)
   - `node renderer/scripts/ensure-css-paths.js`
   - `cd renderer && node switch-env.js web && next build --no-lint` (builds the Next.js app for web)
   - A final PowerShell command to copy the generated `tailwind-web.css` and other assets to the `app/` directory for Firebase deployment.
...
- **`renderer/public/styles/tailwind-web.css`**: This is the fully processed Tailwind CSS output file, generated by `renderer/scripts/generate-tailwind.js`. It is the primary source of Tailwind styles for the web application.

- **`renderer/scripts/generate-tailwind.js`**: Node.js script responsible for processing `globals.css` with PostCSS/Tailwind to produce `tailwind-web.css`.

- **`renderer/utils/styleInjector.js`**: Runtime style injection mechanism for web mode.
...
2. **Web Mode**:
   - Relies on `tailwind-web.css` (generated by `renderer/scripts/generate-tailwind.js` during the `build:web` process) as the primary source of Tailwind styles. This file is processed using `renderer/tailwind.config.js` and `postcss.config.js`.
   - `renderer/utils/styleInjector.js` links this `tailwind-web.css` file at runtime.
   - `styleInjector.js` also injects other minimal critical styles, fonts, and supplementary non-Tailwind-directive based CSS rules (sourced from `globals.css` via `renderer/scripts/sync-styles.js`).
   - Early style embedding in `_document.js` may be used to prevent FOUC for the very minimal injected styles.
